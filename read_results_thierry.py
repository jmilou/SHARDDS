...
#%%
# We read the estimation from sparta along with other keywrds
# This step is not necessary any more, one can just open the final csv files

# Creation of 5 dictionaries to store the mea, median, rms, max and min values
value_mean = {'target_name':target_names,'measured_strehl':sr}
value_median = {'target_name':target_names,'measured_strehl':sr}
value_rms = {'target_name':target_names}
value_max={'target_name':target_names}
value_min={'target_name':target_names}
...
for target_name in target_names:
    ...
    file_sparta_stats = target_name+'_keywords_statistics_F.csv'
    pd_target_sparta_stats = pd.read_csv(os.path.join(path_pipeline,\
                                                      file_sparta_stats))
    fields = list(pd_target_sparta_stats['name'])
    for field in fields:
        if field not in value_mean.keys():
            value_mean[field] = np.ones(nb_targets)*np.nan
        if field not in value_median.keys():
            value_median[field] = np.ones(nb_targets)*np.nan
        if field not in value_rms.keys():
            value_rms[field] = np.ones(nb_targets)*np.nan
        if field not in value_max.keys():
            value_max[field] = np.ones(nb_targets)*np.nan
        if field not in value_min.keys():
            value_min[field] = np.ones(nb_targets)*np.nan


# We populate the 5 dictionaries
for itarget,target_name in enumerate(target_names):
    ...
    pd_target_sparta_stats = pd.read_csv(os.path.join(path_pipeline,\
                                                      file_sparta_stats))
    for ikeyword,keyword in enumerate(pd_target_sparta_stats['name']):
        value_mean[keyword][itarget] = pd_target_sparta_stats['mean'][ikeyword]
        value_median[keyword][itarget] = pd_target_sparta_stats['median'][ikeyword]
        value_rms[keyword][itarget] = pd_target_sparta_stats['rms'][ikeyword]
        value_max[keyword][itarget] = pd_target_sparta_stats['max'][ikeyword]
        value_min[keyword][itarget] = pd_target_sparta_stats['min'][ikeyword]

# Conversion to pandas
pd_mean = pd.DataFrame(value_mean)
pd_median = pd.DataFrame(value_median)
pd_rms = pd.DataFrame(value_rms)
pd_max = pd.DataFrame(value_max)
pd_min = pd.DataFrame(value_min)

# We save everything
pd_mean.to_csv(os.path.join(path,'mean_values.csv'),index=False)
pd_median.to_csv(os.path.join(path,'median_values.csv'),index=False)
pd_rms.to_csv(os.path.join(path,'rms_values.csv'),index=False)
pd_max.to_csv(os.path.join(path,'max_values.csv'),index=False)
pd_min.to_csv(os.path.join(path,'min_values.csv'),index=False)
...
plt.scatter(pd_total['measured_strehl'],pd_median['strehl'],c=,s=500, cmap='gray')
...
plt.grid()
plt.legend()
plt.savefig(os.path.join(path,'comparison_contrast_vs_strehl_cadi.pdf'))
...
plt.grid()
plt.legend()
plt.savefig(os.path.join(path,'comparison_contrast_vs_strehl_pca.pdf'))
...
plt.grid()
plt.legend()
plt.savefig(os.path.join(path,'comparison_raw_coro_contrast_vs_strehl.pdf'))


plt.semilogy(pd_total['tau0_mean']*1000,pd_total['contrast_raw_coro_200mas'],'ro',label='200mas')
plt.semilogy(pd_total['tau0_mean']*1000,pd_total['contrast_raw_coro_500mas'],'bo',label='500mas')
plt.semilogy(pd_total['tau0_mean']*1000,pd_total['contrast_raw_coro_1000mas'],'go',label='1000mas')
...
plt.semilogy(pd_total['old_dimm_seeing_mean'],pd_total['contrast_raw_coro_200mas'],'ro',label='200mas')
plt.semilogy(pd_total['old_dimm_seeing_mean'],pd_total['contrast_raw_coro_500mas'],'bo',label='500mas')
plt.semilogy(pd_total['old_dimm_seeing_mean'],pd_total['contrast_raw_coro_1000mas'],'go',label='1000mas')
...
plt.semilogy(pd_total['seeing_mean'],pd_total['contrast_raw_coro_200mas'],'ro',label='200mas')
plt.semilogy(pd_total['seeing_mean'],pd_total['contrast_raw_coro_500mas'],'bo',label='500mas')
plt.semilogy(pd_total['seeing_mean'],pd_total['contrast_raw_coro_1000mas'],'go',label='1000mas')
...
plt.semilogy(pd_total['wind_speed_mean'],pd_total['contrast_raw_coro_200mas'],'ro',label='200mas')
plt.semilogy(pd_total['wind_speed_mean'],pd_total['contrast_raw_coro_500mas'],'bo',label='500mas')
plt.semilogy(pd_total['wind_speed_mean'],pd_total['contrast_raw_coro_1000mas'],'go',label='1000mas')
...
plt.semilogy(pd_total['wind_speed_mean'],pd_total['contrast_cadi_200mas'],'ro',label='200mas')
plt.semilogy(pd_total['wind_speed_mean'],pd_total['contrast_cadi_500mas'],'bo',label='500mas')
plt.semilogy(pd_total['wind_speed_mean'],pd_total['contrast_cadi_1000mas'],'go',label='1000mas')
plt.xlabel('RTC turbulent wind speed (m/s)')
plt.ylabel('$5\sigma$ post-ADI (classical) contrast')
...
plt.loglog(pd_total['flux_ADU_per_ap_mean'],pd_total['contrast_raw_coro_200mas'],'ro',label='200mas')
plt.loglog(pd_total['flux_ADU_per_ap_mean'],pd_total['contrast_raw_coro_500mas'],'bo',label='500mas')
plt.loglog(pd_total['flux_ADU_per_ap_mean'],pd_total['contrast_raw_coro_1000mas'],'go',label='1000mas')
plt.xlabel('WFS Flux (arbitrary unit)')
plt.ylabel('$5\sigma$ raw coronagraphic contrast')
#plt.axis([0,25,3e-5,5e-2])
plt.grid()
plt.legend()
plt.savefig(os.path.join(path,'SHARDDS_comparison_raw_coro_contrast_vs_WFS_flux.pdf'))


plt.semilogy(pd_total['old_dimm_seeing_rms'],pd_total['contrast_cadi_200mas'],'ro',label='200mas')
plt.semilogy(pd_total['old_dimm_seeing_rms'],pd_total['contrast_cadi_500mas'],'bo',label='500mas')
plt.semilogy(pd_total['old_dimm_seeing_rms'],pd_total['contrast_cadi_1000mas'],'go',label='1000mas')
